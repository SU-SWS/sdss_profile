...
namespace Drupal\Tests\sdss_profile\Unit\Config;

use Drupal\Core\Cache\CacheableMetadata;
use Drupal\Core\Config\Config;
use Drupal\Core\Config\ConfigFactoryInterface;
use Drupal\Core\State\StateInterface;
use Drupal\sdss_profile\Config\ConfigOverrides;
 ...
 *
 * @group sdss_profile
 * @coversDefaultClass \Drupal\sdss_profile\Config\ConfigOverrides
 */
class ConfigOverridesTest extends UnitTestCase {

  /**
   * @var \Drupal\sdss_profile\Config\ConfigOverrides
   */
  ...
  public function getStateCallback($name) {
    switch ($name) {
      case 'sdss_profile.403_page':
        return '/node/403';

      case 'sdss_profile.404_page':
        return '/node/404';

      case 'sdss_profile.front_page':
        return '/node/99';
